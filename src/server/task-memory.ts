...
let loaded=false;let writeQueue=Promise.resolve();
...
async function save(){const job=writeQueue.then(async()=>{await fs.mkdir(dir,{recursive:true});const data=Object.fromEntries(store.entries());await fs.writeFile(file,JSON.stringify(data,null,2),"utf8")});writeQueue=job.catch(()=>{});return job}
