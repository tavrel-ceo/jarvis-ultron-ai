import fs from "node:fs/promises";import path from "node:path";import { publishRealtime } from "./realtime";
...
export async function emitTaskEvent(type:TaskEventType,taskId:string,sessionId:string,message:string,data?:unknown){await load();const event={id:crypto.randomUUID(),type,taskId,sessionId,message,createdAt:Date.now(),read:false,data};events!.push(event);await save();publishRealtime(sessionId,event);return event}
